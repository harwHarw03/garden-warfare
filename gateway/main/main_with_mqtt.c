/**
 * alternative to act as a gateway, from esp_now to cloud / server
 * 
 * this code will resolve shared resources problem when run
 * espnow and wifi AP to connect with mqtt broker 
 * 
*/
...
void on_recv_cb(const esp_now_recv_info_t *packet_info, const uint8_t *packet, int packet_len)
{
    ...
    if (packet_len == sizeof(sensorData_t)) {
        ...
        ESP_LOGE(TAG, "Received : %d", packet_len);
    }

    evt.id = EXAMPLE_ESPNOW_RECV_CB;
    memcpy(recv_cb->mac_addr, packet_info->src_addr, ESP_NOW_ETH_ALEN);

    xSemaphoreTake(xSemaphoreData, portMAX_DELAY);
	recv_cb->data = malloc(packet_len);
	if (recv_cb->data == NULL) {
		ESP_LOGE(TAG, "Malloc receive data fail");
		return;
	}
	memcpy(recv_cb->data, packet, packet_len);
	recv_cb->data_len = packet_len;
...
static void espnow_task(void *pvParameter)
{
	...
	while (xQueueReceive(s_espnow_queue, &evt, portMAX_DELAY) == pdTRUE) {
		if (evt.id == EXAMPLE_ESPNOW_SEND_CB) {
            // not implemented yet, come soon if node need time data from master
			...
			// Send MQTT
			ESP_LOGI(pcTaskGetName(0), "recv_data.soil=[%s]", recv_data.soil_moisture);
			strcpy(mqttBuf.data, recv_data.soil_moisture);
